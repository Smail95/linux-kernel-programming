...
static struct kmem_cache *task_sample_pool;
...
static unsigned long free_tm_list(unsigned long tofree)
{
	...
	list_for_each(entry, &task_m->head){
		...
		if(todel != NULL){
			...
			list_del(todel);
			kmem_cache_free(task_sample_pool, sample);
			freed++;
		}
		todel = entry;
	}
...
static void save_sample(void)
{
	...
	sample = kmem_cache_alloc(task_sample_pool, GFP_KERNEL);
...
{	
	...
	/* Create new cache of <struct task_sample> */
	task_sample_pool = KMEM_CACHE(task_sample, 0);
	if(task_sample_pool == NULL){
